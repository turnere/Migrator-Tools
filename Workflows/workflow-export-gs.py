...
def main():
    all_workflows = []  # List to store all workflow details
    csv_data = []  # List to store CSV rows (workflow name, form ID, form GUID, and form name)
        ...
        if workflow_data:
            ...
            if form_ids:
                for form_id in form_ids:
                    # Fetch additional form details (name and guid)
                    form_data = fetch_form_details(form_id)
                    if form_data:
                        form_guid = form_data.get('guid', 'Unknown GUID')
                        form_name = form_data.get('name', 'Unknown Name')
                        csv_data.append([workflow_name, form_id, form_guid, form_name])
                    else:
                        csv_data.append([workflow_name, form_id, 'Unknown GUID', 'Unknown Name'])
            else:
                csv_data.append([workflow_name, 'No formId found', 'N/A', 'N/A'])
    ...
    if all_workflows:
        with open('workflow_details.json', 'w') as json_file:
            json.dump(all_workflows, json_file, indent=4)  # Save data in pretty JSON format
        logging.info("Workflow data saved to workflow_details.json")
    else:
        logging.info("No workflow data to save.")

    # Write the CSV file with workflow names, form IDs, form GUIDs, and form names
    with open('workflow_formIds_forms.csv', 'w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(['Workflow Name', 'Form ID', 'Form GUID', 'Form Name'])  # Write header row
